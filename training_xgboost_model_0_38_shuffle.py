...
data = pd.read_csv("demo/sample_data.csv", index_col=0)
data = data[data.AC > 0].drop(['Time', 'Hour'], axis=1).reset_index(drop=True)
...
log_folder_name = "Test_R2_HYPEROPT_SHUFFLED"
...
for room in tqdm(room_list):
    ...
    if args.SMOTE:
        # Label all the AC data by 0.38, all AC above 0.38 will be marked as 1, otherwise 0. Split into X and y
        room_data['SMOTE_split'] = (room_data['AC'] > 0.38).astype('int')

        X = room_data.drop(['SMOTE_split'], axis=1)
        y = room_data['SMOTE_split'].sample(frac=1).reset_index(drop=True)
